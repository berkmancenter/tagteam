...
class TagSorter
  ...
  def initialize(args)
    ...
    @klass = args[:class]
    raise 'Must supply context to sort by date created' if (@sort_by == 'created_at') && @context.blank?
    raise 'Must supply class to sort by date created' if (@sort_by == 'created_at') && @klass.blank?
  end
