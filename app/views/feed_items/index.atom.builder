...
#      entry.link( :type => 'text/html', :href => hub_feed_feed_item_url(item.hub_feed_for_hub(@hub.id),item), :rel => 'self' )
      ...
      item.tag_list_on(@hub_feed.hub.tagging_key).each do |tag|
        entry.category(:term => (@hub_feed.hub.tag_prefix.blank?) ? tag : "#{@hub_feed.hub.tag_prefix}#{tag}", :scheme => hub_hub_feed_url(@hub,@hub_feed))
      end
