...
describe FeedItem do
  describe '#copy_tags_to_hubs' do
    context 'a feed item with tag "a" exists in two hubs' do
      ...
      it 'runs on creation' do
        expect(subject).to callback(:copy_tags_to_hubs).after(:create)
      end
      ...
      it 'does not create duplicate taggings' do
        feed_item = create(:feed_item_from_feed, :tagged, tag: @tag, feed: @feed)
        expect{ feed_item.copy_tags_to_hubs }.not_to raise_error
      end
    end
  end
end
